  ...
  useEffect(() => {
    ...
    listSearches()
      ...
      .catch((e) => {
        ...
        setSearchLoadError("Could not load workspace activity. You can still manage workspaces.");
      })
    ...
    <AppShell>
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-6xl mx-auto">
          ...
          {searchLoadError && (
            <p className="mt-3 text-sm text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-500/20 rounded-lg px-3 py-2">
              {searchLoadError}
            </p>
          )}
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {loading && Array.from({ length: 3 }).map((_, i) => (
              <div key={`skeleton-${i}`} className="rounded-2xl border border-gray-200 dark:border-white/10 p-4 bg-white dark:bg-[#111111] animate-pulse">
                <div className="h-5 w-2/3 bg-gray-200 dark:bg-white/10 rounded" />
                <div className="h-4 w-1/3 bg-gray-200 dark:bg-white/10 rounded mt-3" />
                <div className="h-4 w-1/2 bg-gray-200 dark:bg-white/10 rounded mt-2" />
                <div className="h-9 w-24 bg-gray-200 dark:bg-white/10 rounded mt-5" />
              </div>
            ))}
            {workspaces.map((ws) => {
                  ...
                  {editingId === ws.id ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        autoFocus
                      />
                      <button onClick={() => setEditingId(null)} className="text-xs text-gray-500" aria-label="Cancel rename">Cancel</button>
                    </div>
                  ) : (
                    <>
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{stats.count} {stats.count === 1 ? "search" : "searches"}</p>
          ...
          {!loading && workspaces.length === 0 && (
            <p className="mt-6 text-sm text-gray-500 dark:text-gray-400">
              No workspaces yet. Create one to organize your Discover and Validate searches.
            </p>
          )}
        </div>
      </div>
